...
interface ExportData {
  ...
  summary: {
    ...
    byType: {
      prod: { count: number; size: string; sizeBytes: number }
      dev: { count: number; size: string; sizeBytes: number }
      transitive: { count: number; size: string; sizeBytes: number }
    }
  }
  packages: Array<{
    name: string
  ...
  duplicates?: Array<{
    name: string
    versions: Array<{ version: string; size: string; sizeBytes: number }>
    wastedSpace: string
...
/**
 * Generate Markdown report for AI analysis
 */
function generateMarkdownReport(data: ExportData): string {
  ...
  lines.push('')
  lines.push(`> Generated by pkg-analyzer on ${data.analyzedAt}`)
  lines.push('')
  ...
  lines.push(`|---|---------|---------|------|------|`)
  const topPackages = data.packages.slice(0, 20)
  topPackages.forEach((pkg, i) => {
    lines.push(`| ${i + 1} | ${pkg.name} | ${pkg.version} | ${pkg.size} | ${pkg.type} |`)
  })
  lines.push('')
  ...
  if (data.duplicates && data.duplicates.length > 0) {
    ...
    lines.push('')
    lines.push(`Found **${data.duplicates.length}** packages with multiple versions:`)
    lines.push('')
  ...
  if (data.unused && data.unused.length > 0) {
    ...
    lines.push('')
    lines.push(`Found **${data.unused.length}** potentially unused dependencies:`)
    lines.push('')
    lines.push(`| Package | Version | Size | Type |`)
    lines.push(`|---------|---------|------|------|`)
    for (const dep of data.unused) {
      lines.push(`| ${dep.name} | ${dep.version} | ${dep.size} | ${dep.type} |`)
    }
    lines.push('')
  }

  // AI analysis prompt
  lines.push(`## AI Analysis Prompt`)
  lines.push('')
  lines.push(`You can copy the following prompt to ask an AI for optimization suggestions:`)
  lines.push('')
  lines.push('```')
  lines.push(`Please analyze this Node.js project's dependencies and suggest optimizations:`)
  lines.push('')
  lines.push(`- Total size: ${data.summary.totalSize} (${data.summary.totalPackages} packages)`)
  lines.push(`- Production: ${data.summary.byType.prod.count} packages`)
  lines.push(`- Development: ${data.summary.byType.dev.count} packages`)
  ...
  data.packages.slice(0, 10).forEach((pkg, i) => {
    lines.push(`${i + 1}. ${pkg.name}@${pkg.version} - ${pkg.size}`)
  })
  lines.push('')
  lines.push(`Please suggest:`)
  lines.push(`1. Which large packages might have smaller alternatives?`)
  lines.push(`2. How to reduce duplicate package versions?`)
  lines.push(`3. Which dependencies might be unnecessary?`)
  lines.push(`4. Any other optimization opportunities?`)
  lines.push('```')
  lines.push('')
 ...
 */
function exportToFile(data: ExportData, filePath: string): void {
...
export async function analyze(projectPath: string = '.', options: AnalyzeOptions = {}) {
  ...
  if (exportFile || shouldCopy) {
    ...
    const allPackagesSorted = [...packages].sort((a, b) => b.size - a.size)
    const dups = findDuplicates(packages)
    const unusedDeps = findUnused(resolvedPath, packages, deps)
    ...
    if (exportFile) {
      exportToFile(exportData, exportFile)
      console.log(chalk.green(`${figures.tick} Exported to ${chalk.cyan(exportFile)}`))
    }
